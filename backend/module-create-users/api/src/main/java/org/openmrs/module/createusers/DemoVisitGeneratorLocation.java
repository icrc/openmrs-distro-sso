...
public class DemoVisitGeneratorLocation {
	...
	public DemoVisitGeneratorLocation(DemoProviderGenerator providerGenerator, DemoObsGenerator obsGenerator,
	...
	public Visit createDemoVisit(Patient patient, List<VisitType> visitTypes, Location location, boolean shortVisit,
		...
		if (lastVisit == null || lastVisit.getStopDatetime() == null) {
			...
			visitStart = toLocalDateTime(lastVisit.getStopDatetime()).plusDays(randomBetween(0, 365 - 1)).plusMinutes(
			    randomBetween(-(24 * 60 - 1), 24 * 60 - 1));
		}
		...
		if (shortVisit) {
			...
			if (shouldRandomEventOccur(.67)) {
				...
				if (shouldRandomEventOccur(.33)) {
					visit.addEncounter(createDemoLabOrdersEncounter(labEncounter,
					    java.time.Duration.ofMinutes(Math.floorDiv(labStartMinutes, 2)), visitProvider));
				}
			}
			...
			if (shouldRandomEventOccur(.67)) {
				...
				visit.addEncounter(createCovidForm(patient, toDate(lastEncounterTime.plusMinutes(formStartMinutes)),
				    location, visitProvider));
			}
			...
			LocalDateTime dischargeTime = admitTime.plusDays(randomBetween(ADMISSION_DAYS_MIN, ADMISSION_DAYS_MAX))
			        .plusMinutes(randomBetween(-12 * 30, 12 * 60));
			
			for (int i = 0; i < java.time.Duration.between(admitTime, dischargeTime).toDays(); i++) {
				lastEncounterTime = lastEncounterTime.plusDays(1).plusMinutes(randomBetween(-12 * 60, 12 * 60));
				...
				if (shouldRandomEventOccur(.05)) {
					visit.addEncounter(createDemoLabOrdersEncounter(labEncounter,
					    Duration.ofMinutes(Math.floorDiv(labStartMinutes, 2)), visitProvider));
				}
				
				if (shouldRandomEventOccur(.33)) {
					int formStartMinutes = randomBetween(0, Math.min(labStartMinutes, 20));
					
					visit.addEncounter(createCovidForm(patient, toDate(lastEncounterTime.minusMinutes(labStartMinutes)
					        .plusMinutes(formStartMinutes)), location, visitProvider));
				}
			}
		...
		log.info("Created a visit with {} encounters for patient {} starting on {} and ending on {}",
		    new Object[] { visit.getNonVoidedEncounters().size(), patient.getPatientIdentifier(), visit.getStartDatetime(),
	...
	protected Encounter createDemoVitalsEncounter(Patient patient, Date encounterTime, Location location, Provider provider) {
